 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */();const Af=document.querySelector("form"),Of=(document.querySelector("body"),document.querySelector(".user-btn-container")),Df=document.querySelector(".user-name"),Lf=document.querySelector(".signIn"),Mf=document.querySelector(".user-bar-log-out-btn");document.querySelector(".addTo");Mf.addEventListener("click",(function(){(e=Ff,Ui(e).signOut()).then((()=>{yn.Notify.success("Sign-out successful")})).catch((e=>{yn.Notify.failure("An error happened")})).finally((()=>{location.reload()}));var e})),Lf.addEventListener("click",(function(){Af.name.value;const e=Af.email.value,t=Af.password.value;Oo(Ff,e,t).then((e=>{onCloseClick(),yn.Notify.success("Glad you're back again");e.user})).catch((e=>{const t=e.code;e.message;"auth/wrong-password"===t?yn.Notify.failure("Wrong password. Please try again."):"auth/user-not-found"===t?yn.Notify.failure("User not found. Please check your email or sign up."):yn.Notify.failure("An error occurred during sign-in. Please try again later.")}))})),Af.addEventListener("submit",(function(e){e.preventDefault();const t=Af.name.value,n=Af.email.value,i=Af.password.value;Ao(Ff,n,i).then((n=>{const i=n.user,r=i.email;i.photoURL;!function(e,t,n,i){wf(_f(Bf,"users/"+e),{displayName:t,email:n}).then((()=>{console.log("Name added to the database successfully.")})).catch((e=>{console.error("Error adding name to the database:",e)}))}(i.uid,t,r),e.target.reset(),onCloseClick()})).catch((e=>{e.code,e.message}))}));const Uf=Nr({apiKey:"AIzaSyAQxsm1fVLslhxTiwQ3FCGOtjW_RrvvnpE",authDomain:"bookshelf-b7b0a.firebaseapp.com",projectId:"bookshelf-b7b0a",storageBucket:"bookshelf-b7b0a.appspot.com",messagingSenderId:"435762232768",appId:"1:435762232768:web:c27015f9ad01edd0675c25",databaseURL:"https://bookshelf-b7b0a-default-rtdb.firebaseio.com/"}),Ff=function(e=xr()){const t=Er(e,"auth");if(t.isInitialized())return t.getImmediate();const n=function(e,t){const n=Er(e,"auth");if(n.isInitialized()){const e=n.getImmediate();if(Ti(n.getOptions(),null!=t?t:{}))return e;Yr(e,"already-initialized")}return n.initialize({options:t})}(e,{popupRedirectResolver:Ha,persistence:[ea,Uo,Bo]}),i=ii("authTokenSyncURL");if(i){const e=(r=i,async e=>{const t=e&&await e.getIdTokenResult(),n=t&&((new Date).getTime()-Date.parse(t.issuedAtTime))/1e3;if(n&&n>Ga)return;const i=null==t?void 0:t.token;Ya!==i&&(Ya=i,await fetch(r,{method:i?"POST":"DELETE",headers:i?{Authorization:`Bearer ${i}`}:{}}))});!function(e,t,n){Ui(e).beforeAuthStateChanged(t,n)}(n,e,(()=>e(n.currentUser))),function(e,t,n,i){Ui(e).onIdTokenChanged(t,n,i)}(n,(t=>e(t)))}var r;const s=ei("auth");return s&&io(n,`http://${s}`),n}(),Bf=Pf(Uf);var jf,qf,Wf;jf=e=>{if(e){const n=e.uid;console.log(e),e.email,e.photoURL,e.emailVerified,t=n,bf(yf(_f(Pf()),`users/${t}`)).then((e=>{if(e.exists()){const t=e.val();console.log(t.displayName),Of.style.display="block",Df.textContent=`${t.displayName}`}else console.log("No data available")})).catch((e=>{console.error(e)})),function(e){const t=_f(Pf(),`users/${e}/books`);bf(t).then((e=>{if(e.exists()){const t=e.val();console.log(Object.values(t))}else console.log("No shopping list data available")})).catch((e=>{console.error(e)}))}(n)}var t},Ui(Ff).onAuthStateChanged(jf,qf,Wf);function zf(e){const t=Ff.currentUser.uid;try{const n=_f(Bf,`users/${t}/books`);console.log(n),vf(n,{bookId:e}).then((()=>{yn.Notify.success("Book added to user's shopping list successfully.")})).catch((e=>{console.error("Error adding book to user's shopping list:",e)}))}catch(e){console.error("Error adding book to user's shopping list:",e)}}document.querySelectorAll(".book-by-category-list").forEach((e=>{e.addEventListener("click",(function(t){console.log(e),zf(bookId),console.log("Clicked on card with ID:",bookId)}))}));const Hf=Ff.currentUser;console.log(Hf),Tf(_f(Bf,"users"),(function(e){console.log(e.val())}));const Vf=document.querySelector(".user-bar-btn"),$f=document.querySelector(".user-bar-log-out-btn");Vf.addEventListener("click",(function(){$f.classList.toggle("is-open")}));
//# sourceMappingURL=shopping-list.3468c7a2.js.map
